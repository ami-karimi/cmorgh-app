<div class="min-h-screen bg-zinc-950 text-zinc-100 font-sans p-4 sm:p-6 flex justify-center items-start pt-8 pb-16">
    <div class="w-full max-w-xl space-y-6">
        ...
        <div class="relative overflow-hidden bg-zinc-900/80 backdrop-blur-xl border border-zinc-800 rounded-[2.5rem] p-6 shadow-2xl">
            <div class="absolute top-0 right-0 w-48 h-48 bg-orange-500/10 rounded-full blur-3xl pointer-events-none"></div>

            <div class="relative z-10 flex items-center justify-between border-b border-zinc-800/80 pb-5 mb-5">
                <div class="flex items-center gap-4">
                    <div class="w-14 h-14 rounded-2xl bg-zinc-800 border border-zinc-700/50 flex items-center justify-center text-orange-400 font-black text-xl shadow-inner">
                        🔑
                    </div>
                    <div>
                        <span class="text-[10px] font-bold text-zinc-500 uppercase tracking-widest block">اشتراک کاربر</span>
                        <h1 class="text-xl font-black text-white font-mono mt-0.5" dir="ltr">@ {{ $account->username }}</h1>
                    </div>
                </div>

                <span class="px-3 py-1.5 rounded-xl text-xs font-black uppercase tracking-wider border {{ $account->is_enabled ? 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20' : 'bg-rose-500/10 text-rose-400 border-rose-500/20' }}">
                    {{ $account->is_enabled ? '● فعال' : '● غیرفعال' }}
                </span>
            </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div class="bg-zinc-950/60 border border-zinc-800/80 rounded-2xl p-4 flex flex-col justify-between">
                    <span class="text-[11px] font-bold text-zinc-400 mb-2">اعتبار زمانی:</span>
                    <div class="mb-2">
                        @if($isExpired)
